'use client';

import { motion } from 'framer-motion';
import { BlackHole } from './BlackHole';

/**
 * BlackHoleSection Component
 * 
 * "You're Pouring Money Into a Black Hole" section for CliqRise marketing site.
 * Uses the animated BlackHole component exported from Figma.
 * 
 * THEME: Starry Night (dark navy + gold accents)
 */

// Generic icons to avoid trademark issues
const AdPlatformIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6" stroke="currentColor" strokeWidth="2">
    <rect x="3" y="3" width="18" height="18" rx="2" />
    <path d="M9 9h6M9 12h6M9 15h4" />
  </svg>
);

const ShoppingIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6" stroke="currentColor" strokeWidth="2">
    <path d="M6 6h15l-1.5 9h-12z" />
    <circle cx="9" cy="20" r="1" />
    <circle cx="18" cy="20" r="1" />
    <path d="M6 6L5 3H2" />
  </svg>
);

export default function BlackHoleSection() {
  return (
    <section className="relative py-20 lg:py-32 overflow-hidden bg-[#1a1c2e]">
      {/* Section Header */}
      <div className="container mx-auto px-6 mb-12 lg:mb-16">
        <motion.h2 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="text-3xl md:text-4xl lg:text-5xl font-serif text-center text-[#e4e8f4]"
        >
          You're Pouring Money Into a{' '}
          <span className="text-[#c75a78]">Black Hole</span>
        </motion.h2>
      </div>

      {/* Main Content - 3 Column Layout */}
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-8 lg:gap-4 items-center">
          
          {/* LEFT: Money Out */}
          <motion.div 
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="flex flex-col items-center lg:items-end text-center lg:text-right"
          >
            <span className="text-xs uppercase tracking-widest text-[#7a839e] mb-4">
              Money Out
            </span>
            
            {/* Ad Platform Card */}
            <div className="bg-[#2a2d47] border border-[#3a3d57] rounded-xl p-6 w-full max-w-xs">
              {/* Platform Icons */}
              <div className="flex justify-center lg:justify-end gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-[#1877f2] flex items-center justify-center text-white">
                  <AdPlatformIcon />
                </div>
                <div className="w-10 h-10 rounded-lg bg-[#e60023] flex items-center justify-center text-white">
                  <AdPlatformIcon />
                </div>
              </div>
              
              {/* Amount */}
              <div className="text-4xl font-bold text-[#c75a78] mb-2">
                $4,200
              </div>
              <div className="text-sm text-[#7a839e]">
                Social Ad Spend
              </div>
              
              {/* Details */}
              <div className="mt-4 pt-4 border-t border-[#3a3d57] text-sm text-[#7a839e]">
                <div>12 campaigns</div>
                <div>47 ad sets</div>
              </div>
            </div>

            {/* Arrow indicator - Mobile */}
            <div className="lg:hidden mt-6 text-[#c75a78]">
              <svg className="w-8 h-8 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
              </svg>
            </div>
          </motion.div>

          {/* CENTER: Black Hole Animation */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.8 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: 0.4 }}
            className="relative flex flex-col items-center"
          >
            {/* Responsive sizing for the black hole */}
            <div className="block md:hidden">
              <BlackHole size={280} />
            </div>
            <div className="hidden md:block lg:hidden">
              <BlackHole size={350} />
            </div>
            <div className="hidden lg:block">
              <BlackHole size={400} />
            </div>

            {/* Label */}
            <div className="mt-4 text-center">
              <span className="text-xs uppercase tracking-widest text-[#c75a78] font-medium">
                The Black Hole
              </span>
              <p className="text-sm text-[#7a839e] mt-1">
                No connection. No tracking.
              </p>
            </div>
          </motion.div>

          {/* RIGHT: Money In */}
          <motion.div 
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="flex flex-col items-center lg:items-start text-center lg:text-left"
          >
            <span className="text-xs uppercase tracking-widest text-[#7a839e] mb-4">
              Money In
            </span>
            
            {/* Earnings Card */}
            <div className="bg-[#2a2d47] border border-[#3a3d57] rounded-xl p-6 w-full max-w-xs">
              {/* Platform Icon */}
              <div className="flex justify-center lg:justify-start gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-[#ff9900] flex items-center justify-center text-white">
                  <ShoppingIcon />
                </div>
              </div>
              
              {/* Amount */}
              <div className="text-4xl font-bold text-[#ffd666] mb-2">
                $8,947
              </div>
              <div className="text-sm text-[#7a839e]">
                Affiliate Earnings
              </div>
              
              {/* Details */}
              <div className="mt-4 pt-4 border-t border-[#3a3d57] text-sm text-[#7a839e]">
                <div>4 storefronts</div>
                <div>342 orders</div>
              </div>
            </div>

            {/* Arrow indicator - Mobile */}
            <div className="lg:hidden mt-6 text-[#ffd666] rotate-180">
              <svg className="w-8 h-8 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
              </svg>
            </div>
          </motion.div>
        </div>
      </div>

      {/* Bottom Pain Points */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6, delay: 0.6 }}
        className="container mx-auto px-6 mt-12 lg:mt-16"
      >
        <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8 text-sm">
          <div className="flex items-center gap-2 text-[#c75a78]">
            <span className="text-lg">✗</span>
            <span>Wasted Spend: Killing winners. Feeding losers.</span>
          </div>
          <div className="flex items-center gap-2 text-[#c75a78]">
            <span className="text-lg">✗</span>
            <span>Hours Lost: Manual spreadsheets. Always outdated.</span>
          </div>
        </div>
      </motion.div>
    </section>
  );
}